...
async function getTokenURI(provider, domainName) {
	// prepare domain name
	const lh = namehash.hash(domainName);
	let contract = new ethers.Contract(Registrar, REVERSE_REGISTRAR_ABI, provider);
	try {
		let res = await contract.tokenURI(ethers.BigNumber.from(lh).toString());
		return res;
	} catch (err) {
